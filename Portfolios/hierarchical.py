...
hierarchical_PFdyn = np.zeros((len(returns.index)-estLength,nAssets)) #initializes matrix for min var portfolio
...
for n in range(0,(len(returns.index)-estLength)):
    ...
    hierarchical = getHRP(estSigma)
    hierarchical_PFdyn[n,:] = np.array([float(x) for x in hierarchical])
...
#    retAM[n,:] = np.array(np.multiply(hierarchical, (np.exp(np.asmatrix(histRet))-1)))
#    retPFM[n,:] = retAM[n,:].sum()

    
hierarchicalPortfolios = pd.DataFrame(hierarchical_PFdyn, index = datesPF, columns = indices) #format data as dataframe with dates and column names
checkoneminvar = hierarchicalPortfolios.sum(axis=1) #check if weights sum to 1

os.chdir("/Users/%s/OneDrive/Master Thesis/Data/Portfolios/" %name)
hierarchicalPortfolios.to_csv('hierarchicalPortfolios{}{}.csv'.format(freq, years))
