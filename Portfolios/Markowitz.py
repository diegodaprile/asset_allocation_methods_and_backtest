...
#    MVPF = np.array(minVarPF1(estSigma))
...
#    minVarPFdyn[n,:] = MVPF
    ...
    maxSloPF = maxSRPF_noshort(meanRet, estSigma, rf)
    maxSlopePFdyn[n,:] = maxSloPF.T       
#    tangPF = maxSRPF1(meanRet, estSigma, rf)
#    maxSharpePFdyn[n,:] = tangPF    
...
#checkoneMV = MVPortfolios.sum(axis=1) #check if weights sum to 1
checkoneMS = MSPortfolios.sum(axis=1) #check if weights sum to 1
#checkoneMSharpe = MSHARPEPortfolios.sum(axis=1)
