...
LPM_PFdyn = np.zeros((len(returns.index)-estLength,nAssets)) #initializes matrix for min var portfolio
...
for n in range(0,(len(returns.index)-estLength)):
    ...
    LPM = lpm_port_noshort(meanRet, df_estimation.values, exp_ret_chosen = 0.062 / 12)
    LPM_PFdyn[n,:] = np.array([float(x) for x in LPM])
...
LPMPortfolios = pd.DataFrame(LPM_PFdyn, index = datesPF, columns = indices) #format data as dataframe with dates and column names
checkoneminvar = LPMPortfolios.sum(axis=1) #check if weights sum to 1

os.chdir("/Users/%s/OneDrive/Master Thesis/Data/Portfolios/no_short_sale/" %name)
LPMPortfolios.to_csv('LPM_Portfol_6.2%_M{}.csv'.format(years))
